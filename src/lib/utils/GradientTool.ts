...
export class MultiStopGradientTool {
  ...
  addStopFromEvent(e: fabric.IEvent) {
  ...
  setActiveStop(stop: fabric.Circle) {
    ...
    this.stops.forEach((s) =>
      s.set("stroke", s === stop ? "#000" : undefined)
    );
  ...
  toggleGradientType() {
    this.gradientType =
      this.gradientType === "linear" ? "radial" : "linear";
    this.update();
  }
  ...
  syncFromObject(obj: fabric.Object) {
    ...
    this.gradientType = obj.fill.type as "linear" | "radial";
